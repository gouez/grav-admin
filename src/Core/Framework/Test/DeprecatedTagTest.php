...
class DeprecatedTagTest extends TestCase
{
     ...
     */
    private function getLaserVersion(): string
    {
        $envVersion = $_SERVER['VERSION'] ?? $_SERVER['TAG'] ?? '';
        if (\is_string($envVersion) && $envVersion !== '') {
            $laserVersion = $envVersion;
        } elseif (InstalledVersions::isInstalled('laser/platform')) {
            $laserVersion = InstalledVersions::getVersion('laser/platform');
        } else {
            $laserVersion = InstalledVersions::getVersion('laser/core');
        }
        $laserVersion = ltrim((string) $laserVersion, 'v ');

        if (!preg_match('/^\d+\.\d+[.-].*$/', $laserVersion)) {
