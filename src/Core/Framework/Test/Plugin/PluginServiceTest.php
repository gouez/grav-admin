...
class PluginServiceTest extends TestCase
{
    ...
    public function testRefreshPluginWithoutExtraLabelProperty(): void
    {
        ...
        $composerJsonException = $errors->filter(fn (LaserHttpException $error) => $error instanceof PluginComposerJsonInvalidException);

        static::assertNotEmpty($composerJsonException);

        $errorFound = false;
        $errorString = 'Plugin composer.json has invalid "type" (must be "laser-platform-plugin"), or invalid "extra/laser-plugin-class" value, or missing extra.label property';
    ...
    private function assertPluginMetaInformation(PluginEntity $plugin): void
    {
        ...
        static::assertSame('laser AG', $plugin->getAuthor());
        static::assertSame('(c) by laser AG', $plugin->getCopyright());
