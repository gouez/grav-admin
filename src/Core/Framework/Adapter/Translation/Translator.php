...
use Laser\Core\System\Snippet\SnippetService;
...
class Translator extends AbstractTranslator
{
    ...
    public function getCatalogue(?string $locale = null): MessageCatalogueInterface
    {
        ...
        if ($this->isLaserLocaleCatalogue($catalog) && !$this->isFallbackLocaleCatalogue($catalog, $localization)) {
            ...
            // or locale is symfony style locale so we shouldn't add laser fallbacks as it may lead to circular references
            $fallbackLocale = null;
        }
    ...
    /**
     * Laser uses dashes in all locales
     * if the catalogue does not contain any dashes it means it is a symfony fallback catalogue
     * in that case we should not add the laser fallback catalogue as it would result in circular references
     */
    private function isLaserLocaleCatalogue(MessageCatalogueInterface $catalog): bool
    {
