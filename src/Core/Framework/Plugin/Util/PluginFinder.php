...
use Laser\Core\Framework\Plugin\Composer\PackageProvider;
use Laser\Core\Framework\Plugin\Exception\ExceptionCollection;
use Laser\Core\Framework\Plugin\Exception\PluginComposerJsonInvalidException;
use Laser\Core\Framework\Plugin\Struct\PluginFromFileSystemStruct;
...
class PluginFinder
{
    final public const COMPOSER_TYPE = 'laser-platform-plugin';
    private const SHOPWARE_PLUGIN_CLASS_EXTRA_IDENTIFIER = 'laser-plugin-class';
    ...
    private function loadLocalPlugins(string $pluginDir, IOInterface $composerIO, ExceptionCollection $errors): array
    {
        ...
        try {
            ...
            foreach ($filesystemPlugins as $filesystemPlugin) {
                ...
                if (!$this->isLaserPluginType($package) || !$this->isPluginComposerValid($package)) {
    ...
    private function isLaserPluginType(CompletePackageInterface $package): bool
    {
    ...
    private function loadVendorInstalledPlugins(
        ...
        foreach ($composerPackages as $composerPackage) {
            if (!$this->isLaserPluginType($composerPackage)) {
                continue;
            }
        ...
        if ($this->isLaserPluginType($root) && $this->isPluginComposerValid($root)) {
