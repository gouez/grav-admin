...
use Laser\Core\Framework\Plugin\Exception\PluginChangelogInvalidException;
use Laser\Core\Framework\Plugin\Exception\PluginComposerJsonInvalidException;
use Laser\Core\Framework\Plugin\Exception\PluginNotFoundException;
...
class PluginService
{
    ...
    public function refreshPlugins(Context $laserContext, IOInterface $composerIO): ExceptionCollection
    {
        ...
        $installedPlugins = $this->getPlugins(new Criteria(), $laserContext);
        ...
        foreach ($pluginsFromFileSystem as $pluginFromFileSystem) {
            ...
            $pluginData['translations'] = $this->getTranslations($laserContext, $extra);
        ...
        if ($plugins !== []) {
            foreach ($plugins as $plugin) {
                try {
                    $this->pluginRepo->upsert([$plugin], $laserContext);
                } catch (LaserHttpException $exception) {
        ...
        if (\count($deletePluginIds) !== 0) {
            ...
            $this->pluginRepo->delete($deletePlugins, $laserContext);
        }
    ...
    private function getTranslations(Context $context, array $extra): array
    {
         ...
         * @example payload
         * {
         *     "laser-plugin-class":"Swag\\MyDemoData\\MyDemoData",
         *     "label":{
