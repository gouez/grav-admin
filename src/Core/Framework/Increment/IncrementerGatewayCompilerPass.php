...
namespace Laser\Core\Framework\Increment;
...
class IncrementerGatewayCompilerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        ...
        $services = $container->getParameter('laser.increment');
        $tag = 'laser.increment.gateway';

        foreach ($services as $pool => $service) {
            $type = $service['type'] ?? null;

            if (!\is_string($type)) {
                throw new \RuntimeException(sprintf('laser.increment.gateway type of %s pool must be a string', $pool));
            }

            $active = sprintf('laser.increment.%s.gateway.%s', $pool, $type);
            $config = [];
    ...
    {
        // laser.increment.gateway.mysql is fallback gateway if custom gateway is not set
        $fallback = sprintf('laser.increment.gateway.%s', $type);
        ...
        switch ($type) {
            ...
            case 'redis':
                ...
                $adapter = sprintf('laser.increment.%s.redis_adapter', $pool);
