...
namespace Laser\Core\Framework\Update\Api;

use Laser\Core\Framework\Context;
use Laser\Core\Framework\Log\Package;
use Laser\Core\Framework\Plugin\KernelPluginLoader\StaticKernelPluginLoader;
use Laser\Core\Framework\Store\Services\AbstractExtensionLifecycle;
use Laser\Core\Framework\Update\Checkers\LicenseCheck;
use Laser\Core\Framework\Update\Checkers\WriteableCheck;
use Laser\Core\Framework\Update\Event\UpdatePostPrepareEvent;
use Laser\Core\Framework\Update\Event\UpdatePrePrepareEvent;
use Laser\Core\Framework\Update\Services\ApiClient;
use Laser\Core\Framework\Update\Services\ExtensionCompatibility;
use Laser\Core\Framework\Update\Steps\DeactivateExtensionsStep;
...
class UpdateController extends AbstractController
{
    ...
    public function updateApiCheck(): JsonResponse
    {
        ...
        if (version_compare($this->laserVersion, $updates->version, '>')) {
            return new JsonResponse();
        }
    ...
    public function deactivatePlugins(Request $request, Context $context): JsonResponse
    {
        ...
        if ($offset === 0) {
            ...
            $this->eventDispatcher->dispatch(
                new UpdatePrePrepareEvent($context, $this->laserVersion, $update->version)
            );
        }
        ...
        if ($result->getOffset() === $result->getTotal()) {
            ...
            $containerWithoutPlugins->get('event_dispatcher')->dispatch(
                new UpdatePostPrepareEvent($context, $this->laserVersion, $update->version)
            );
        }
