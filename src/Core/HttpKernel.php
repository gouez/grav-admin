...
use Laser\Core\Framework\Adapter\Database\MySQLFactory;
use Laser\Core\Framework\Event\BeforeSendRedirectResponseEvent;
use Laser\Core\Framework\Event\BeforeSendResponseEvent;
use Laser\Core\Framework\Log\Package;
use Laser\Core\Framework\Plugin\KernelPluginLoader\DbalKernelPluginLoader;
use Laser\Core\Framework\Plugin\KernelPluginLoader\KernelPluginLoader;
use Laser\Core\Framework\Routing\CanonicalRedirectService;
use Laser\Core\Framework\Routing\RequestTransformerInterface;
use Laser\Core\Profiling\Doctrine\ProfilingMiddleware;
use Laser\Storefront\Framework\Cache\CacheStore;
...
class HttpKernel
{
    ...
    private function doHandle(Request $request, int $type, bool $catch): HttpKernelResult
    {
        ...
        $enabled = $container->hasParameter('laser.http.cache.enabled')
            && $container->getParameter('laser.http.cache.enabled');
    ...
    private function createKernel(): KernelInterface
    {
        ...
        if (InstalledVersions::isInstalled('laser/platform')) {
            $laserVersion = InstalledVersions::getVersion('laser/platform')
                . '@' . InstalledVersions::getReference('laser/platform');
        } else {
            $laserVersion = InstalledVersions::getVersion('laser/core')
