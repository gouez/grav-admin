...
class SystemSetupCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('jwt-passphrase', null, InputOption::VALUE_OPTIONAL, 'JWT private key passphrase', 'laser')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (file_exists($this->projectDir . '/symfony.lock')) {
            $io->warning('It looks like you have installed Laser with Symfony Flex. You should use a .env.local file instead of creating a complete new one');
        }

        $io->title('Laser setup process');
    ...
    private function getDsn(InputInterface $input, SymfonyStyle $io): array
    {
        ...
        $dbName = $io->ask('Database name', 'laser', $emptyValidation);
