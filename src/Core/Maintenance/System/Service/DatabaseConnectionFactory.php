...
namespace Laser\Core\Maintenance\System\Service;
...
class DatabaseConnectionFactory
{
    ...
    {
        // https://developer.laser.com/docs/guides/installation/overview#system-requirements
        ...
        if (\mb_stripos((string) $version, 'mariadb') !== false) {
            if (version_compare($version, $mariaDBRequiredVersion, '<')) {
                throw new DatabaseSetupException(sprintf(
                    'Your database server is running MariaDB %s, but Laser 6 requires at least MariaDB %s OR MySQL %s',
                    $version,
        ...
        if (version_compare($version, $mysqlRequiredVersion, '<')) {
            throw new DatabaseSetupException(sprintf(
                'Your database server is running MySQL %s, but Laser 6 requires at least MySQL %s OR MariabDB %s',
                $version,
                $mysqlRequiredVersion,
                $mariaDBRequiredVersion
            ));
