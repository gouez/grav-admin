...
class SetupDatabaseAdapter
{
    ...
    public function initializeLaserDb(Connection $connection, ?string $database = null): bool
    {
        if (!$hasLaserTables = $this->hasLaserTables($connection, $database)) {
            $connection->executeStatement($this->getBaseSchema());
        }

        return !$hasLaserTables;
    }

    public function hasLaserTables(Connection $connection, ?string $database = null): bool
    {
        if ($database) {
