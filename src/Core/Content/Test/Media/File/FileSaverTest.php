...
use Laser\Core\Content\Media\Event\MediaFileExtensionWhitelistEvent;
use Laser\Core\Content\Media\Exception\CouldNotRenameFileException;
use Laser\Core\Content\Media\Exception\DuplicatedMediaFileNameException;
use Laser\Core\Content\Media\Exception\FileExtensionNotSupportedException;
use Laser\Core\Content\Media\Exception\MediaNotFoundException;
use Laser\Core\Content\Media\Exception\MissingFileException;
...
class FileSaverTest extends TestCase
{
    ...
    public function testRenameMediaMakesRollbackOnFailure(): void
    {
        ...
        $allowed = $this->getContainer()->getParameter('laser.filesystem.allowed_extensions');
        /** @var list<string> $allowedPrivate */
        $allowedPrivate = $this->getContainer()->getParameter('laser.filesystem.private_allowed_extensions');

        $fileSaverWithFailingRepository = new FileSaver(
            $repositoryMock,
            $this->getContainer()->get('laser.filesystem.public'),
