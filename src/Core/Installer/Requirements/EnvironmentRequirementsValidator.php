...
use Laser\Core\Installer\Requirements\Struct\SystemCheck;
...
class EnvironmentRequirementsValidator implements RequirementsValidatorInterface
{
    ...
    {
        $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('laser/platform', '*');
        if (!$platform) {
            $platform = $this->composer->getRepositoryManager()->getLocalRepository()->findPackage('laser/core', '*');
        }
        if (!$platform) {
