...
class ServiceReferenceGenerator implements ScriptReferenceGenerator
{
    ...
    final public const GITHUB_BASE_LINK = 'https://github.com/laser/platform/blob/trunk';
    ...
    {
        if (!str_starts_with($className, 'Laser\\') || !\class_exists($className)) {
            return null;
        }
    ...
    private function findScriptServices(): array
    {
        ...
        $laserClasses = ConstructFinder::locatedIn(__DIR__ . '/../../../..')
            ->exclude('*/Test/*', '*/vendor/*', '*/DevOps/StaticAnalyze*')
            ->findClassNames();

        foreach ($laserClasses as $class) {
